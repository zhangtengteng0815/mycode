...
BrandingText "犀思云 https://www.syscxp.com"

DirText "安装向导将把犀思云 VPN 安装在下列文件夹，如果要安装到其他文件夹请单击 [浏览(B)] 进行选择。"
...
!define MUI_WELCOMEPAGE_TEXT "犀思云 VPN"
...
!define MUI_COMPONENTSPAGE_TEXT_TOP "选择要安装/升级的组件。停止任何 ${PACKAGE_NAME} 进程或 ${PACKAGE_NAME} 服务（如果正在运行）。所有DLL都在本地安装。"
...
!define MUI_ICON "admin2.ico"
;!define MUI_UNICON "icon.ico"
!define MUI_UNICON "admin2.ico"
...
LangString DESC_SecOpenVPNUserSpace ${LANG_ENGLISH} "安装 ${PACKAGE_NAME} 用户空间组件，包括 openvpn.exe 。"

;LangString DESC_SecOpenVPNGUI ${LANG_ENGLISH} "Install ${PACKAGE_NAME} GUI by Mathias Sundman"
LangString DESC_SecOpenVPNGUI ${LANG_ENGLISH} "安装犀思云 VPN 的GUI"

;LangString DESC_SecTAP ${LANG_ENGLISH} "Install/upgrade the TAP virtual device driver."
LangString DESC_SecTAP ${LANG_ENGLISH} "安装/升级 TAP 虚拟设备驱动程序。"

;LangString DESC_SecOpenVPNEasyRSA ${LANG_ENGLISH} "Install EasyRSA 2 scripts for X509 certificate management."
LangString DESC_SecOpenVPNEasyRSA ${LANG_ENGLISH} "安装用于 X509 证书管理的 EasyRSA 2 脚本。"
...
LangString DESC_SecOpenSSLDLLs ${LANG_ENGLISH} "在本地安装 OpenSSL DLL（如果 DLL 已经全局安装，则可以省略）。"

;LangString DESC_SecLZODLLs ${LANG_ENGLISH} "Install LZO DLLs locally (may be omitted if DLLs are already installed globally)."
LangString DESC_SecLZODLLs ${LANG_ENGLISH} "在本地安装 LZO DLL（如果 DLL 已被全局安装，则可以省略）。"
...
LangString DESC_SecPKCS11DLLs ${LANG_ENGLISH} "在本地安装 PKCS＃11 帮助程序 DLL（如果 DLL 已被全局安装，则可以省略）。"

;LangString DESC_SecService ${LANG_ENGLISH} "Install the ${PACKAGE_NAME} service wrappers"
LangString DESC_SecService ${LANG_ENGLISH} "安装 ${PACKAGE_NAME} 服务包装。"
...
LangString DESC_SecInteractiveService ${LANG_ENGLISH} "安装 ${PACKAGE_NAME} 交互式服务（允许在没有管理员权限的情况下运行犀思云-VPN）。"
...
Function CoreSetup
	...
	${If} ${SectionIsSelected} ${SecAddShortcutsWorkaround}
		...
		;CreateShortCut "$SMPROGRAMS\${PACKAGE_NAME}\Utilities\Generate a static ${PACKAGE_NAME} key.lnk" "$INSTDIR\bin\openvpn.exe" '--pause-exit --verb 3 --genkey --secret "$INSTDIR\config\key.txt"' "$INSTDIR\admin2.ico" 0
...
Section /o "犀思云" SecOpenVPNGUI

	SetOverwrite on
	SetOutPath "$INSTDIR\bin"


	File "${OPENVPN_ROOT_X86_64}\bin\syscxp-gui.exe"


	${If} ${SectionIsSelected} ${SecAddShortcutsWorkaround}
		CreateDirectory "$SMPROGRAMS\${PACKAGE_NAME}"
		CreateShortCut "$SMPROGRAMS\${PACKAGE_NAME}\犀思云.lnk" "$INSTDIR\bin\syscxp-gui.exe" ""
		CreateShortcut "$DESKTOP\犀思云.lnk" "$INSTDIR\bin\syscxp-gui.exe"
	${EndIf}
	...
	${If} ${SectionIsSelected} ${SecLaunchGUIOnLogon0}
		WriteRegStr HKLM "Software\Microsoft\Active Setup\Installed Components\${PACKAGE_NAME}_UserSetup" "StubPath" "reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Run /v OPENVPN-GUI /t REG_SZ /d $\"$INSTDIR\bin\syscxp-gui.exe$\" /f"
	${Else}
...
SectionGroup "!Advanced"
	...
	Section /o "${PACKAGE_NAME} File Associations" SecFileAssociation
		...
		WriteRegStr HKCR "${PACKAGE_NAME}File\DefaultIcon" "" "$INSTDIR\admin2.ico,0"
	...
	Section /o "Add Shortcuts to Start Menu" SecAddShortcuts
		...
		CreateShortCut "$SMPROGRAMS\${PACKAGE_NAME}\卸载犀思云.lnk" "$INSTDIR\Uninstall.exe"
	SectionEnd
...
Section -post
	...
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PACKAGE_NAME}" "DisplayName" "Syscxp-gui"
	...
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PACKAGE_NAME}" "DisplayIcon" "$INSTDIR\admin2.ico"
	;WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PACKAGE_NAME}" "DisplayVersion" "${VERSION_STRING}"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PACKAGE_NAME}" "DisplayVersion" "1.0"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PACKAGE_NAME}" "HelpLink" "https://www.syscxp.com/"
	...
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PACKAGE_NAME}" "Publisher" "The syscxp developers"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PACKAGE_NAME}" "UninstallString" "$INSTDIR\Uninstall.exe"
	WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PACKAGE_NAME}" "URLInfoAbout" "https://www.syscxp.com/"
...
Section "Uninstall"
	...
	Delete "$INSTDIR\bin\syscxp-gui.exe"
