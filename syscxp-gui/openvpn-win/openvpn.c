    ...
    *pos = '\0';
    ...
    if (strcmp(state, "CONNECTED") == 0 && strcmp(message, "SUCCESS") == 0)
    {
        ...
        ||  (c->state == reconnecting && o.show_balloon == 2))
        {
            ...
            LoadLocalizedStringBuf(msg, _countof(msg), IDS_NFO_NOW_CONNECTED);
...
UserAuthDialogFunc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDOK:{
            ...
            {
                wcscpy(secretPassword, password);
            ...
            SetDlgItemTextW(hwndDlg, ID_TXT_WARNING, LoadLocalizedString(IDS_NFO_USER_LOGIN));
            if(!save_key()){
                SetDlgItemTextW(hwndDlg, ID_TXT_WARNING, LoadLocalizedString(IDS_NFO_USER_PASS_RETRY));
                return 0;
            }

            SetDlgItemTextW(hwndDlg, ID_TXT_WARNING, LoadLocalizedString(IDS_NFO_CONNECTING));
            if(! cxp_start()){
                SetDlgItemTextW(hwndDlg, ID_TXT_WARNING, LoadLocalizedString(IDS_NFO_CONNECT_RETRY));
                return 0;
            };

            if(syscxp_echo==0){

                if(strcmp(configStatus, "Initial") == 0){
                    while(1){
                        ce_echo();
                        if(get_Config_Ce_Cmd){
                            break;
                        }
                    }
                }
            ...
            WCHAR ovpnpass[USER_PASS_LEN];
            swprintf(ovpnpass, USER_PASS_LEN, L"%hs", secret_key);
            SetDlgItemTextW(hwndDlg, ID_OVPN_PASS, ovpnpass);

            //write_log_file("ovpnpassword========>>>>>>>>>>>>>>>>>>>>>>>>>>>");
            //write_log_file(ovpnpass);

            if (param->flags & FLAG_CR_TYPE_SCRV1)
                ManagementCommandFromInputBase64(param->c, "password \"Auth\" \"SCRV1:%s:%s\"", hwndDlg, ID_OVPN_PASS, ID_EDT_AUTH_CHALLENGE);
            else
                ManagementCommandFromInput(param->c, "password \"Auth\" \"%s\"", hwndDlg, ID_OVPN_PASS);
        ...
        case IDCANCEL:{
            ...
            WCHAR cmdline[] = L"TASKKILL  /F  /IM  openvpn.exe";
            ...
            //system("TASKKILL  /F  /IM openvpn.exe");
            //IsProcess("openvpn.exe","o");
...
BOOL
ManagementCommandFromSecretKey(connection_t *c, LPCSTR fmt,char * s)
{
    BOOL retval = FALSE;
    LPSTR cmd;
    int cmd_len;
    ...
    cmd_len = strlen(s) + strlen(fmt);
    cmd = malloc(cmd_len);

    if (cmd)
    {
        snprintf(cmd, cmd_len, fmt, s);
        ...
        write_log_file("==========ManagementCommand success===========");
...
OnPassword(connection_t *c, char *msg)
{
    ...
    write_log_file("OnPassword with msg===>>>>");
    write_log_file(msg);
    ...
    if (strstr(msg, "'Auth'"))
    {
        ...
        else
        {
            if(login_times==1){
                ...
                sprintf(secret_key_str, "%s", secret_key );
                //strcpy(secret_key_str,secret_key);

                write_log_file("======send user========");
                write_log_file(esnstr);
                write_log_file(secret_key_str);
                write_log_file("=========send pass===============");

                ManagementCommandFromSecretKey(param->c,"username \"Auth\" \"%s\"",esnstr);
                ManagementCommandFromSecretKey(param->c,"password \"Auth\" \"%s\"",secret_key_str);
...
OnStop(connection_t *c, UNUSED char *msg)
{
    ...
    case timedout:
        ...
        MessageBox(c->hwndStatus, LoadLocalizedString(msg_id, msg_xtra), LoadLocalizedString(IDS_TIP_DEFAULT), MB_OK);
...
RenderStatusWindow(HWND hwndDlg, UINT w, UINT h)
{
        ...
        MoveWindow(GetDlgItem(hwndDlg, ID_TXT_STATUS), DPI_SCALE(20), DPI_SCALE(10), w-DPI_SCALE(30), DPI_SCALE(15), TRUE);
        MoveWindow(GetDlgItem(hwndDlg, ID_TXT_IP), DPI_SCALE(20), DPI_SCALE(30), w-DPI_SCALE(30), DPI_SCALE(15), TRUE);
        MoveWindow(GetDlgItem(hwndDlg, ID_TXT_BYTECOUNT), DPI_SCALE(20),DPI_SCALE(50), DPI_SCALE(400), DPI_SCALE(15), TRUE);
        //MoveWindow(GetDlgItem(hwndDlg, ID_TXT_VERSION), w-DPI_SCALE(180), h - DPI_SCALE(55), DPI_SCALE(170), DPI_SCALE(15), TRUE);
        MoveWindow(GetDlgItem(hwndDlg, ID_DISCONNECT), DPI_SCALE(20), DPI_SCALE(70), DPI_SCALE(110), DPI_SCALE(23), TRUE);
        MoveWindow(GetDlgItem(hwndDlg, ID_RESTART), DPI_SCALE(145), DPI_SCALE(70), DPI_SCALE(110), DPI_SCALE(23), TRUE);
        //MoveWindow(GetDlgItem(hwndDlg, ID_HIDE), w - DPI_SCALE(130), h - DPI_SCALE(30), DPI_SCALE(110), DPI_SCALE(23), TRUE);
        MoveWindow(GetDlgItem(hwndDlg, ID_HIDE), DPI_SCALE(270), DPI_SCALE(70), DPI_SCALE(110), DPI_SCALE(23), TRUE);
}
...
StatusDialogFunc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        HWND hLogWnd = CreateWindowEx(0, RICHEDIT_CLASS, NULL,
            ...
            0, 0, 0, 0, hwndDlg, (HMENU) ID_EDT_LOG, o.hInstance, NULL);
        if (!hLogWnd)
        {
...
ThreadOpenVPNStatus(void *p)
{
    ...
    SetWindowText(c->hwndStatus, LoadLocalizedString(IDS_NFO_CONNECTION_XXX));
...
StartOpenVPN(connection_t *c)
{
    ...
    //GetRandomPassword(secret_key, sizeof(secret_key) - 1);
   ...
   /* char tmp_ca[2000];
    ...
    ReplaceSubStr(syscxp_ca,src,dst,tmp_ca);
    ReplaceSubStr(syscxp_cert,src,dst,tmp_cert);
    ReplaceSubStr(syscxp_key,src,dst,tmp_key);*/

    TCHAR t_ca[2000];
    TCHAR t_cert[5000];
    TCHAR t_key[2000];

    strcpy(t_ca, syscxp_ca);
    strcpy(t_cert, syscxp_cert);
    strcpy(t_key, syscxp_key);

    //write_log_file(syscxp_ca);
    //write_log_file(syscxp_cert);
    //write_log_file(syscxp_key);


    _sntprintf_0(cmdline, _T("openvpn ~~log%s \"%s\" "
        "~~setenv IV_GUI_VER \"%S\" ~~service %s 0 ~~auth-retry interact "
        "~~management %S %hd stdin ~~management-query-passwords %s "
        "~~management-hold  ~~client ~~auth-user-pass  ~~dev tap ~~proto udp "
        "~~remote \"%S\" \"%S\" ~~remote \"%S\" \"%S\" ~~ping 5  ~~comp-lzo ~~nobind ~~cipher \"%S\" ~~verb 4 ~~mute 20 "
        " ~~ca \"[[INLINE]]\"  \"%S\"  ~~cert  \"[[INLINE]]\"  \"%S\"  ~~key  \"[[INLINE]]\"  \"%S\" "),
        (o.log_append ? _T("-append") : _T("")), c->log_path,
        PACKAGE_STRING, exit_event_name,
        inet_ntoa(c->manage.skaddr.sin_addr), ntohs(c->manage.skaddr.sin_port),
        (o.proxy_source != config ? _T("~~management-query-proxy ") : _T("")), master_remote_ip,master_remote_port,slave_remote_ip,slave_remote_port,
        syscxp_cipher,t_ca,t_cert,t_key);

    /*_sntprintf_0(cmdline, _T("openvpn ~~log%s \"%s\" ~~auth-user-pass \"C:\\pass.txt\" "
        "~~setenv IV_GUI_VER \"%S\" ~~service %s 0 ~~auth-retry interact "
        "~~management %S %hd stdin ~~management-query-passwords %s "
        "~~management-hold  ~~client ~~dev tap ~~proto udp "
        "~~remote \"%S\" \"%S\" ~~remote \"%S\" \"%S\" ~~ping 5  ~~comp-lzo ~~nobind ~~cipher \"%S\" ~~verb 4 ~~mute 20 "
        " ~~ca \"[[INLINE]]\"  \"%S\"  ~~cert  \"[[INLINE]]\"  \"%S\"  ~~key  \"[[INLINE]]\"  \"%S\" "),
        (o.log_append ? _T("-append") : _T("")), c->log_path,
        PACKAGE_STRING, exit_event_name,
    ...
    //write_log_file("master_remote_ip======>>>>>>");
    //write_log_file(master_remote_ip);
    //write_log_file("cmdline========>>>>>>>>");
    char str111[50000];
    strcpy(str111, cmdline);
    //write_log_file(str111);
    ...
    if (!IsUserAdmin() && InitServiceIO (&c->iserv))
    {
        ...
        //DWORD size = _tcslen(c->config_dir) + _tcslen(options) + sizeof(secret_key) + 3;
        ...
        /*_sntprintf_0(startup_info, L"%s%c%s%s%c%.*S", c->config_dir, L'\0',
            options, extra_options, L'\0', sizeof(secret_key), secret_key);
...
//        si.wShowWindow = SW_HIDE;
        ...
        //write_log_file("============password==========");
        //write_log_file(c->manage.password);
        //write_log_file("==========password============");
        ...
        /*secret_key[sizeof(secret_key) - 1] = '\n';
        WriteFile(hStdInWrite, secret_key, sizeof(secret_key), &written, NULL);
        secret_key[sizeof(secret_key) - 1] = '\0';*/
