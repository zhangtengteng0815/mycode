...
HTTP_API const char* ft_http_sync_post_request(ft_http_client_t* http, const char* url, const char* post_data, http_request_method_e m)
{
	if (http == NULL)
	{
		return NULL;
	...
	http->error_code = http_internal_sync_request(http, url, post_data, strlen(post_data), 0, 0);
...
int is_http_error(ft_http_client_t* http){
